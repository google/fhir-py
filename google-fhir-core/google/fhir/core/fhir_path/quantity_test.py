...
class QuantityTest(absltest.TestCase):
  ONE_GRAM = quantity.Quantity(value=1, unit='g')
  TWO_GRAMS = quantity.Quantity(value=2, unit='g')
  ZERO_GRAMS = quantity.Quantity(value=0, unit='g')
  ONE_KILOGRAM = quantity.Quantity(value=1, unit='kg')

  def test_eq_ne(self):
    self.assertEqual(self.ONE_GRAM, quantity.Quantity(value=1, unit='g'))
  ...
  def test_lt(self):
    ...
    self.assertFalse(self.ONE_GRAM < quantity.Quantity(value=1, unit='g'))
    self.assertFalse(self.ONE_GRAM < self.ZERO_GRAMS)
    with self.assertRaisesRegex(NotImplementedError, 'Units must be the same'):
      self.ONE_GRAM.__lt__(self.ONE_KILOGRAM)
    self.assertEqual(self.ONE_GRAM.__lt__(1), NotImplemented)
  ...
  def test_gt(self):
    ...
    self.assertEqual(self.ONE_GRAM.__gt__(1), NotImplemented)

  def test_le(self):
    self.assertLessEqual(self.ONE_GRAM, self.TWO_GRAMS)
    self.assertLessEqual(self.ONE_GRAM, quantity.Quantity(value=1, unit='g'))
    # pylint: disable=g-generic-assert
    self.assertFalse(self.ONE_GRAM <= self.ZERO_GRAMS)
    with self.assertRaisesRegex(NotImplementedError, 'Units must be the same'):
      self.ONE_GRAM.__le__(self.ONE_KILOGRAM)
    self.assertEqual(self.ONE_GRAM.__le__(1), NotImplemented)

  def test_ge(self):
    self.assertGreaterEqual(self.ONE_GRAM, self.ZERO_GRAMS)
    self.assertGreaterEqual(self.ONE_GRAM, quantity.Quantity(value=1, unit='g'))
    # pylint: disable=g-generic-assert
    self.assertFalse(self.ONE_GRAM >= self.TWO_GRAMS)
    with self.assertRaisesRegex(NotImplementedError, 'Units must be the same'):
      self.ONE_GRAM.__ge__(self.ONE_KILOGRAM)
    self.assertEqual(self.ONE_GRAM.__ge__(1), NotImplemented)

  def test_str(self):
    self.assertEqual(str(quantity.Quantity(value=1, unit='g')), "1 'g'")
